<script lang="ts">
  ...
  import { state,Post,Forg } from "./store";
  let inp = "";
  let Orgs:Array<any> = [];
  let Sl:Array<any> = [];
  let Slp:Array<any> = [];
      ...
      <Command.List>
        ...
        {#if Slp.length > 0}
          ...
          <div class="flex flex-col w-full">
            {#each Slp as elem}
              <Button on:click={()=>{Post.set(elem);Slp=[];Sl=[];state.set("seepost")}} variant="link" class="w-full">{elem.title}</Button>
            {/each}
          </div>
        {/if}
        <Command.Separator />
        {#if Sl.length > 0}
          ...
          <div class="flex flex-col w-full">
            {#each Sl as elem}
              <Button variant="link" on:click={()=>{Slp=[];Sl=[];Forg.set(elem);state.set("review")}} class="w-full">{elem.organisation}</Button>
            {/each}
          </div>
        {/if}
      </Command.List>
